...
Crypto.prototype.bootstrapSecretStorage = async function ({
  ...
  const crossSigningInfo = new _CrossSigning.CrossSigningInfo(this._userId, builder.crossSigningCallbacks, builder.crossSigningCallbacks); // the ID of the new SSSS key, if we create one
  ...
  if (!inStorage && !keyBackupInfo) {
    ...
    if (setupNewSecretStorage || !oldKeyInfo || oldKeyInfo.algorithm !== _SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES) {
    ...
    _logger.logger.log("Secret storage default key not found, using key backup key"); // if we have the backup key already cached, use it; otherwise use the
    // callback to prompt for the key


    const backupKey = (await this.getSessionBackupPrivateKey()) || (await getKeyBackupPassphrase()); // create new cross-signing keys

    await resetCrossSigning(); // create a new SSSS key and use the backup key as the new SSSS key
    ...
    _logger.logger.log("Cross-signing private keys found in secret storage"); // TODO: take this use case out of bootstrapping
  ...
  if (crossSigningPrivateKeys.size) {
    _logger.logger.log("Storing cross-signing private keys in secret storage"); // Assuming no app-supplied callback, default to storing in SSSS.
  ...
  if (setupNewKeyBackup && !keyBackupInfo) {
    ...
    await crossSigningInfo.signObject(data.auth_data, "master"); // sign with the device fingerprint
...
Crypto.prototype.checkOwnCrossSigningTrust = async function () {
  ...
  if (oldSelfSigningId !== newCrossSigning.getId("self_signing")) {
    _logger.logger.info("Got new self-signing key", newCrossSigning.getId("self_signing")); // Try to cache the self-signing private key as a side-effect
    ...
    try {
      ...
      _logger.logger.info("Got matching private key from callback for new public self-signing key");
    } finally {
      if (signing) signing.free();
    }
  ...
  if (oldUserSigningId !== newCrossSigning.getId("user_signing")) {
    _logger.logger.info("Got new user-signing key", newCrossSigning.getId("user_signing")); // Try to cache the user-signing private key as a side-effect
    ...
    try {
      ...
      _logger.logger.info("Got matching private key from callback for new public user-signing key");
    } finally {
      if (signing) signing.free();
    }
  }
