...
class EncryptionSetupOperation {
  ...
  async apply(crypto) {
    const baseApis = crypto._baseApis; // set account data
    ...
    if (this._crossSigningKeys) {
      ...
      await baseApis.uploadDeviceSigningKeys(this._crossSigningKeys.auth, keys); // pass the new keys to the main instance of our own CrossSigningInfo.
